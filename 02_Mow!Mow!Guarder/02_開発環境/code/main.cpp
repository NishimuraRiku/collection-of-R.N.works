...
//****************************************
// ウインドウの名前
#define WINDOW_NAME	"Mow!Mow!Guarder"
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR IpCmdLine, int nCmdShow)
{
	...
	WNDCLASSEX wcex =
	{
		...
		(HICON)LoadImage(				// タスクバーのアイコン
			NULL,"data\\ICON\\icon_000.ico",IMAGE_ICON,0,0,LR_SHARED | LR_LOADFROMFILE),
		LoadCursor(NULL,IDC_ARROW),		// マウスカーソル
		(HBRUSH)(COLOR_WINDOW + 1),		// クライアント領域の背景色
		NULL,							// メニューバー
		CLASS_NAME,						// ウインドウクラスの名前
		(HICON)LoadImage(				// ファイルのアイコン
			NULL,"data\\ICON\\icon_000.ico",IMAGE_ICON,0,0,LR_SHARED | LR_LOADFROMFILE),
	};
	...
	// ウインドウの表示
	WindowModeChange(GetSetting()->bFullScreen);
...
void Uninit(void)
{
	...
	UninitSound();			// サウンド
	UninitFade();			// フェード
	UninitUi_screen_00();	// UI :スクリーン [00]
...
void Update(void)
{
	...
	UpdateLight();				// 光
	UpdateFade();				// フェード
	UpdateUi_screen_00();		// UI :スクリーン [00]
...
void WindowModeChange(bool bFullScreen)
{
	...
	{// 全画面フラグが偽の時、
		...
		// ウインドウスタイルを変更
		SetWindowLong(g_main.hWnd, GWL_STYLE, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX);
	}
...
void DrawComment(char *pString)
{
	...
	// テキストの描画(右下)
	rect = { 0 + 1, g_main.nCommentY + 1, SCREEN_WIDTH, SCREEN_HEIGHT };
	g_main.pFont->DrawText(NULL, pString, -1, &rect, DT_LEFT, D3DCOLOR_RGBA(0, 0, 0, 255));
	rect = { 0 + 2, g_main.nCommentY + 2, SCREEN_WIDTH, SCREEN_HEIGHT };
	g_main.pFont->DrawText(NULL, pString, -1, &rect, DT_LEFT, D3DCOLOR_RGBA(0, 0, 0, 255));

	// テキストの描画
	rect = { -1, g_main.nCommentY , SCREEN_WIDTH, SCREEN_HEIGHT };
	g_main.pFont->DrawText(NULL, pString, -1, &rect, DT_LEFT, D3DCOLOR_RGBA(g_main.commentCol.r, g_main.commentCol.g, g_main.commentCol.b, 255));
