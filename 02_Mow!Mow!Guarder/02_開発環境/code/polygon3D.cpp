...
{
	// サウンドのアニメーションの読み込み
	if (GetMd_game_00()->state != MD_GAME_00_STATE_CLEAR)
	{// MD:ゲーム画面[00] の状態がクリア演出でない時、
		for (int nCntSoundAnim = 0; nCntSoundAnim < motionSet.aMotion[nMotionNum].nSoundAnimNum; nCntSoundAnim++)
		{
			...
			{// モーションカウンターが切り替え時間に達した時、
				// サウンドを再生
				PlaySound((SOUND_LABEL)motionSet.aMotion[nMotionNum].aSoundAnim[nCntSoundAnim].nSound);
			}
		}
	}

	// 振動のアニメーションの読み込み
	for (int nCntVibAnim = 0; nCntVibAnim < motionSet.aMotion[nMotionNum].nVibAnimNum; nCntVibAnim++)
	{
		if (pPartsInfo->nCounterMotion == motionSet.aMotion[nMotionNum].aVibAnim[nCntVibAnim].nTime)
		{// モーションカウンターが切り替え時間に達した時、
			// カメラ(3D)の振動を設定
			SetCamera3DVibration(motionSet.aMotion[nMotionNum].aVibAnim[nCntVibAnim].fVib);
		}
	}
...
void LoadMotion3DSet(FILE *pFile, Motion3DSet *pMotionSet)
{
	...
	while (1)
	{
		...
		if		(!strcmp(aDataSearch, "MOTIONSET_END")) { break; }	// 読み込みを終了
			...
			// 初期化
			pMotion->nSoundAnimNum = 0;	// サウンドのアニメーション数
			pMotion->nVibAnimNum = 0;	// 振動のアニメーション数
			...
			while (1)
			{
				...
				else if (!strcmp(aDataSearch, "VIB_ANIM")) {
					// 振動のアニメーションの情報の読み込みを開始
					while (1)
					{
						fscanf(pFile, "%s", aDataSearch); // 検索

						if (!strcmp(aDataSearch, "VIB_ANIM_END"))	{ break; }						// 読み込みを終了
						else if (!strcmp(aDataSearch, "TIME"))		{								// 振動のアニメーション情報
							fscanf(pFile, "%d", &pMotion->aVibAnim[pMotion->nVibAnimNum].nTime);	// 切り替え時間
							fscanf(pFile, "%f", &pMotion->aVibAnim[pMotion->nVibAnimNum].fVib);		// 振動の強さ
							pMotion->nVibAnimNum++;													// 振動のアニメーション数を加算
						}
					}
				}
			}
...
//========================================
// SetTexturePos3DCut関数 - テクスチャ座標の設定処理(3D)(切り取りver) -
//========================================
void SetTexturePos3DCut(VERTEX_3D *pVtx, Scale cutScale, D3DXVECTOR3 pos)
{
	pos.x += 0.5f;
	pos.y += 0.5f;
