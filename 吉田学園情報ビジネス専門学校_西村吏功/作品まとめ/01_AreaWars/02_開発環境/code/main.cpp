...
// マクロ定義
//****************************************
#define WINDOW_NAME	"AreaWars"	// ウインドウの名前
...
MODE g_mode = MODE_TITLE_NONE;			// 現在のモード
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR IpCmdLine, int nCmdShow)
{
	...
	// カーソルの表示/非表示
	ShowCursor(!GetSetting()->bFullScreen);
...
HRESULT Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	InitCamera();		// カメラ
	InitBg_color();		// BG :背景色
	InitUi_cursor_00();	// UI :カーソル	[00]
	InitUi_frame_00();	// UI :フレーム	[00]
	InitSound(hWnd);	// サウンド
	InitSetting();		// 設定

	// フェードの設定
	InitFade(MODE_TITLE_00);
...
void Uninit(void)
{
	...
	UninitMd_title_00();		// MD :タイトル画面	[00]
	UninitMd_game_00();			// MD :ゲーム画面	[00]
	UninitUi_cursor_00();		// UI :カーソル		[00]
...
{
	// 読み込み処理
	LoadEff_general_00();	// EFF:エフェクト全般	[00]
	LoadAtk_bullet_00();	// ATK:弾				[00]
	LoadChr_fighter_00();	// CHR:機体				[00]
...
void DrawDebugComment(void)
{
	...
	Chr_fighter_00 *pChr_fighter_00 = GetChr_fighter_00();

	sprintf(aString, "FPS：%d\n", g_nCountFPS);
	sprintf(aString, "%sX:%.1f Y:%.1f\n", aString,pChr_fighter_00->pos.x,pChr_fighter_00->pos.y);
	sprintf(aString, "%srot:%f\n", aString, pChr_fighter_00->rot.z);
