	...
	for (int nCntType = 0; nCntType < OBJ_STAGE_00_TYPE_NUM; nCntType++)
	{
		for (int nCntParts = 0; nCntParts < PARTS_3D_MAX; nCntParts++)
		{
			...
			for (int nCntMat = 0; nCntMat < MATERIAL_3D_MAX; nCntMat++) {
				if (g_aTextureObj_stage_00[nCntType][nCntParts][nCntMat] != NULL)
				{
					g_aTextureObj_stage_00[nCntType][nCntParts][nCntMat]->Release();
					g_aTextureObj_stage_00[nCntType][nCntParts][nCntMat] = NULL;
				}
			}
		}
	}
