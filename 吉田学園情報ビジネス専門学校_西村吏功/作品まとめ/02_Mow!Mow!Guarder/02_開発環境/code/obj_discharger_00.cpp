...
void UninitObj_discharger_00(void)
{
	...
	for (int nCntType = 0; nCntType < OBJ_DISCHARGER_00_TYPE_NUM; nCntType++)
	{
		...
		for (int nCntMat = 0; nCntMat < MATERIAL_3D_MAX; nCntMat++) {
			if (g_aTextureObj_discharger_00[nCntType][nCntMat] != NULL)
			{
				g_aTextureObj_discharger_00[nCntType][nCntMat]->Release();
				g_aTextureObj_discharger_00[nCntType][nCntMat] = NULL;
			}
		}
	}
