...
// グローバル宣言
//****************************************
LPDIRECT3DTEXTURE9	g_aTextureObj_target_00		// OBJ:ターゲット[00] のテクスチャへのポインタ
					[MATERIAL_3D_MAX]
					= {};
...
void InitObj_target_00(void)
{
	...
	for (int nCntMat = 0; nCntMat < (int)g_dwNumMatObj_target_00; nCntMat++) 
	{
		...
		{
			// テクスチャの読み込み
			D3DXCreateTextureFromFile(pDevice, pMat[nCntMat].pTextureFilename, &g_aTextureObj_target_00[nCntMat]);
		}
	}
}
...
void UninitObj_target_00(void)
{
	...
	for (int nCntMat = 0; nCntMat < MATERIAL_3D_MAX; nCntMat++) {
		if (g_aTextureObj_target_00[nCntMat] != NULL)
		{
			g_aTextureObj_target_00[nCntMat]->Release();
			g_aTextureObj_target_00[nCntMat] = NULL;
		}
	}
}
...
void DrawObj_target_00(void)
{
	...
	for (int nCntMat = 0; nCntMat < (int)g_dwNumMatObj_target_00; nCntMat++) 
	{
		...
		// テクスチャの設定
		pDevice->SetTexture(0, g_aTextureObj_target_00[nCntMat]);
