	...
	bool bRflc, bool bSlip)
{
	...
	if (pCollision != NULL) {
		...
		{// 全ての方向から内側の時、
			pCollision->bInside = true;	// 範囲内フラグを真にする
			if (!pairHitTestParts.aGhost[HIT_DIRECTION_BACK] ||
				!pairHitTestParts.aGhost[HIT_DIRECTION_FRONT] ||
				!pairHitTestParts.aGhost[HIT_DIRECTION_LEFT] ||
				!pairHitTestParts.aGhost[HIT_DIRECTION_RIGHT])
			{
				pCollision->bHit = true;	// 当たりフラグを真にする
			}
		}
