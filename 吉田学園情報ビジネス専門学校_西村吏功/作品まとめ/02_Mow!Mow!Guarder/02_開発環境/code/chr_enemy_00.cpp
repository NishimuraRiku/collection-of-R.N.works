	...
	for (int nCntType = 0; nCntType < CHR_ENEMY_00_TYPE_MAX; nCntType++)
	{
		for (int nCntParts = 0; nCntParts < PARTS_3D_MAX; nCntParts++)
		{
			...
			for (int nCntMat = 0; nCntMat < MATERIAL_3D_MAX; nCntMat++) {
				if (g_aTextureChr_enemy_00[nCntType][nCntParts][nCntMat] != NULL) 
				{
					g_aTextureChr_enemy_00[nCntType][nCntParts][nCntMat]->Release();
					g_aTextureChr_enemy_00[nCntType][nCntParts][nCntMat] = NULL;
				}
			}
		}
	}
