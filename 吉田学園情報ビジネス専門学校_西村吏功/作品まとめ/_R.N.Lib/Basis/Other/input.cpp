...
{
	// カーソルの画面上の位置を取得
	D3DXVECTOR3 cursorPos = GetCursorPosOnScreen();

	if (g_inputInfo.cursorPosTemp != cursorPos)
	{// カーソルの現在位置が保存位置を異なる時、
		// 動的なデバイスをキーボードにする
		g_activeInputType = ACTIVE_DEVICE_KEYBOARD;
	}
	...
	// カーソルの移動量を設定
	g_inputInfo.cursorMove = cursorPos - g_inputInfo.cursorPosTemp;

	// カーソル位置を保存
	g_inputInfo.cursorPosTemp = cursorPos;
}
...
{
	// スティックの入力情報
	BYTE aAngle[STICK_TYPE_MAX][STICK_ANGLE_MAX];
	...
	for (int nCntStick = 0; nCntStick < STICK_TYPE_MAX; nCntStick++)
	{
		...
		{
			aAngle[nCntStick][nCntAngle] = false;
		}
		...
		{// スティックが倒されている時、
			...
			{// 角度が四分割で上に位置する時、上フラグを真にする
				aAngle[nCntStick][STICK_ANGLE_UP] = true;
			}
			...
			{// 角度が四分割で下に位置する時、下フラグを真にする
				aAngle[nCntStick][STICK_ANGLE_DOWN] = true;
			}
			else if ((g_inputInfo.stick.aAngle[nCntStick] > D3DX_PI * -0.75)
				&& (g_inputInfo.stick.aAngle[nCntStick] < D3DX_PI * -0.25))
			{// 角度が四分割で左に位置する時、左フラグを真にする
				aAngle[nCntStick][STICK_ANGLE_LEFT] = true;
			}
			else if ((g_inputInfo.stick.aAngle[nCntStick] > D3DX_PI * 0.25)
				&& (g_inputInfo.stick.aAngle[nCntStick] < D3DX_PI * 0.75))
			{// 角度が四分割で右に位置する時、右フラグを真にする
				aAngle[nCntStick][STICK_ANGLE_RIGHT] = true;
			}
		}
		...
		{
			// スティックのトリガー情報を保存
			g_inputInfo.stick.aAngleTrigger[nCntStick][nCntAngle] = (g_inputInfo.stick.aAnglePress[nCntStick][nCntAngle] ^ aAngle[nCntStick][nCntAngle])&aAngle[nCntStick][nCntAngle];

			// スティックのリリース情報を保存
			g_inputInfo.stick.aAngleRelease[nCntStick][nCntAngle] = (g_inputInfo.stick.aAnglePress[nCntStick][nCntAngle] ^ aAngle[nCntStick][nCntAngle])&~aAngle[nCntStick][nCntAngle];

			// 現在の時間を取得
			g_inputInfo.aStickCurrentTime[nCntStick][nCntAngle] = timeGetTime();

			if (aAngle[nCntStick][nCntAngle] && ((g_inputInfo.aStickCurrentTime[nCntStick][nCntAngle] - g_inputInfo.aStickExecLastTime[nCntStick][nCntAngle]) > REPEATE_INTERVAL))
			{// キーが入力されていて、かつ現在の時間と最後に真を返した時間の差がリピートの間隔を越えていた時、
				// 最後に真を返した時間を保存
				g_inputInfo.aStickExecLastTime[nCntStick][nCntAngle] = g_inputInfo.aStickCurrentTime[nCntStick][nCntAngle];

				// スティックのリピート情報を保存
				g_inputInfo.stick.aAngleRepeat[nCntStick][nCntAngle] = aAngle[nCntStick][nCntAngle];
			}
			else {
				// スティックのリピート情報を保存
			...
			// スティックのプレス情報を保存
			g_inputInfo.stick.aAnglePress[nCntStick][nCntAngle] = aAngle[nCntStick][nCntAngle];

			if (aAngle[nCntStick][nCntAngle])
			{// 入力が行われた時、
				// 動的なデバイスをコントローラーにする
				g_activeInputType = ACTIVE_DEVICE_CONTROLLER;
			}
...
{
	return g_inputInfo.aMouseState[nMouse] != 0;
}
...
{
	return g_inputInfo.aMouseStateTrigger[nMouse] != 0;
}
...
{
	return g_inputInfo.aMouseStateRepeat[nMouse] != 0;
}
...
{
	return g_inputInfo.aMouseStateRelease[nMouse] != 0;
}
...
{
	return g_inputInfo.aButtonPress[nButton] != 0;
}
...
{
	return g_inputInfo.aButtonTrigger[nButton] != 0;
}
...
{
	return g_inputInfo.aButtonRepeat[nButton] != 0;
}
...
{
	return g_inputInfo.aButtonRelease[nButton] != 0;
}
