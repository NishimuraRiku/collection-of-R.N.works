...
#define CHR_ENEMY_00_CLEAR_TIME						(40)
	...
	for (int nCntAct = 0; nCntAct < g_aChr_enemy_00Type[pChr->nType].nActNum; nCntAct++)
	{
		...
		{// 行動カウンター行動する時間と一致した時、
			...
			case CHR_ENEMY_00_ACT_LOAD_TURN_AROUND_TO_PLAYER_START:
				...
				// 方向転換位置を設定
				pChr->pTurnAroundPos = &GetChr_fighter_00()->pos;
			...
			case CHR_ENEMY_00_ACT_LOAD_TRACKING_PLAYER_START:
				...
				// 追尾位置を設定
				pChr->pTrackingPos = &GetChr_fighter_00()->pos;
			...
			case CHR_ENEMY_00_ACT_LOAD_STARE_PLAYER_START:
				...
				// 凝視位置を設定
				pChr->pStarePos = &GetChr_fighter_00()->pos;
			...
			case CHR_ENEMY_00_ACT_LOAD_STARE_CENTER_START:
				...
				// 凝視位置を設定
				pChr->pStarePos = &GetObj_plasma_block_00_square()->centerPos;
...
void CollisionChr_fighter_00(Chr_enemy_00 *pChr)
{
	...
	if (CheckHit(
		...
		g_aChr_enemy_00Type[pChr->nType].hitTestType,
		GetChr_fighter_00()->pos,
		GetChr_fighter_00()->rot,
		GetChr_fighter_00()->fHitTestWidth,
		GetChr_fighter_00()->fHitTestHeight,
		GetChr_fighter_00Type()[GetChr_fighter_00()->nType].hitTestType))
	{// 敵[00] の座標が機体[00] の当たり判定内に入った時、
		// 機体[00] のヒット処理
		HitChr_fighter_00(g_aChr_enemy_00Type[pChr->nType].nHitDamage);

		if (g_aChr_enemy_00Type[pChr->nType].bHitBreak)
		{// 衝突時の破壊処理が真の時、
			// 敵[00] の破壊処理
			DestructionChr_enemy_00(pChr);
		}
		else if (pChr->aAct[CHR_ENEMY_00_ACT_KNOCK_BACK])
		{// 衝突時の破壊処理が偽で、かつノックバック状態の時、
			// 相手の機体[00] までの角度に移動量を加算
			GetChr_fighter_00()->move.x += sinf(FindAngle(GetChr_fighter_00()->pos, pChr->pos) + D3DX_PI) * (CHR_ENEMY_00_KNOCK_BACK_POWER);
			GetChr_fighter_00()->move.y += cosf(FindAngle(GetChr_fighter_00()->pos, pChr->pos) + D3DX_PI) * (CHR_ENEMY_00_KNOCK_BACK_POWER);
		}
	}
}
						...
						{// 発射する時間を読み込む
							...
							if (strcmp(aDataSearch, "[") == 0) 
							{
								while (1)
								{
									...
									{// 弾の種類を読み込む
										fscanf(pFile, "%d", &pChrType->aShot[pChrType->nShotNum].nBulletType); // 検索
									}//====================//
...
void SetChr_enemy_00(D3DXVECTOR3 pos, D3DXVECTOR3 rot, int nType, bool bAreaBoss, bool bBomb, int nParentNum, int nChildNum)
{
	...
	for (int nCntChr = 0; nCntChr < CHR_ENEMY_00_MAX; nCntChr++, pVtx += 4, pChr++)
	{
		...
		pChr->bAreaBoss		= bAreaBoss;	// エリアボスフラグを代入
...
void HitChr_enemy_00(Chr_enemy_00 *pChr, int nDamage)
{
	...
	{// 体力が0以下の時、
		...
		{// 爆発を受けたフラグが真の時、
			// 膨らみ待ち状態にする
			pChr->aState[CHR_ENEMY_00_STATE_SWELL_WAIT] = true;
			pChr->aCounterState[CHR_ENEMY_00_STATE_SWELL_WAIT] = 0;
		}
		else
		{// 爆発を受けたフラグが偽の時、
			// 膨らみ状態にする
