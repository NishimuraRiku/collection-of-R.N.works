...
LPD3DXMESH				g_aMeshChr_player_00		// CHR:プレイヤー[00] のメッシュ(頂点情報)へのポインタ
						...
						[PARTS_3D_MAX]
						= {};
LPD3DXBUFFER			g_pBuffMatChr_player_00		// CHR:プレイヤー[00] のマテリアルへのポインタ
						= NULL;
	...
	for (int nCntType = 0; nCntType < CHR_PLAYER_00_TYPE_NUM; nCntType++)
	{
		...
		for (int nCntParts = 0; nCntParts < pType->partsSet.nPartsNum; nCntParts++)
		{
			...
			D3DXLoadMeshFromX(
				...
				pDevice,
				NULL,
				&g_pBuffMatChr_player_00,
				NULL,
				&g_aNumMatChr_player_00[nCntType][nCntParts],
				&g_aMeshChr_player_00[nCntType][nCntParts]);

			// マテリアル情報に対するポインタを取得
			pMat = (D3DXMATERIAL*)g_pBuffMatChr_player_00->GetBufferPointer();
...
void UninitChr_player_00(void)
{
	...
	// マテリアルの破棄
	if (g_pBuffMatChr_player_00 != NULL)
	{
		g_pBuffMatChr_player_00->Release();
		g_pBuffMatChr_player_00 = NULL;
	}
}
