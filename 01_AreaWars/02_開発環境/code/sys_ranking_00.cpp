...
// プロトタイプ宣言
//****************************************
void ScoreSortDesc	(int *nScore, int *nUpdateRank);	// スコアランキングを降順にソートする
...
{
	int		nTemp;			// 一時的に値を格納
	char	aNameTemp		// 名前を一時的に格納
			[RANKING_NAME_NUM + 1];	
	int		nCntCompare1,	// 比較のカウント
			nCntCompare2;
	bool	bUpdateRank		// 順位更新フラグ
			= false;
	...
	for (nCntCompare1 = 0; nCntCompare1 < RANKING_NUM - 1; nCntCompare1++)
	{
		for (nCntCompare2 = nCntCompare1 + 1; nCntCompare2 < RANKING_NUM; nCntCompare2++)
		{
			...
			{// 要素2が要素1の値を上回っていた時、
				...
				// 名前を入れ替える
				sprintf(aNameTemp, "%s",g_sys_ranking_00[nCntCompare1].aName);
				sprintf(g_sys_ranking_00[nCntCompare1].aName, "%s", g_sys_ranking_00[nCntCompare2].aName);
				sprintf(g_sys_ranking_00[nCntCompare2].aName, "%s", aNameTemp);
