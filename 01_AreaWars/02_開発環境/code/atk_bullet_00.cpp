...
#define ATK_BULLET_00_ROTATION_TiME			(20)
#define ATK_BULLET_00_TYPE_DATA_FILE_PATH	"data\\GAMEDATA\\ATK_BULLET_00_DATA.txt"
...
// 当たり判定処理
void CollisionAtk_bullet_00(Atk_bullet_00 *pAtk);
...
// グローバル宣言
//****************************************
LPDIRECT3DTEXTURE9		g_aTextureAtk_bullet_00		//
						[ATK_BULLET_00_TYPE_MAX]	//
						= {};						// テクスチャへのポインタ
LPDIRECT3DVERTEXBUFFER9	g_pVtxBuffAtk_bullet_00		//
						= NULL;						// 頂点バッファへのポインタ
Atk_bullet_00Type		g_aAtk_bullet_00Type		//
						[ATK_BULLET_00_TYPE_MAX];	// 弾[00] の種類毎の情報
Atk_bullet_00			g_aAtk_bullet_00			//
						[ATK_BULLET_00_MAX];		// 弾[00] の情報
...
//========================================
// CollisionAtk_bullet_00関数 - 当たり判定処理 -
//========================================
void CollisionAtk_bullet_00(Atk_bullet_00 *pAtk) 
{
	// 親の種類に応じた当たり判定処理
...
void SetParticle(Atk_bullet_00 *pAtk)
{
	...
	for (int nCntPtcl = 0; nCntPtcl < g_aAtk_bullet_00Type[pAtk->nType].nPtclNum; nCntPtcl++)
	{
		...
		// パーティクル[00] を設定
		SetEff_particle_00(pAtk->pos, setRot, g_aAtk_bullet_00Type[pAtk->nType].aPtclType);
	}
}
...
void SetLight(Atk_bullet_00 *pAtk)
{
	...
	SetEff_light_00(
		...
		g_aAtk_bullet_00Type[pAtk->nType].aLightColor,
	...
	while (1)
	{
		...
		{// 弾[00] の種類毎の情報の読み込みを開始
			while (1)
			{
				...
				{// テクスチャの相対パスを読み込む
					fscanf(pFile, "%s", pAtkType->aTexturPath);
				}//====================//
				...
				{// 当たり判定の種類を読み込む
					...
					// 当たり判定の種類を文字列から読み込む
					StringLoadHitTestType(aDataSearch, &pAtkType->hitTestType);
				}//====================//
				...
				else if (strcmp(aDataSearch, "ライトの色:") == 0)
				{// ライトの色を読み込む
					fscanf(pFile, "%d", &pAtkType->aLightColor.nR);
					fscanf(pFile, "%d", &pAtkType->aLightColor.nG);
					fscanf(pFile, "%d", &pAtkType->aLightColor.nB);
					fscanf(pFile, "%d", &pAtkType->aLightColor.nA);
				}//====================//
				...
				else if (strcmp(aDataSearch, "パーティクルの種類:") == 0)
				{// パーティクルの種類を読み込む
					fscanf(pFile, "%d", &pAtkType->aPtclType);
				}//====================//
...
void InitAtk_bullet_00(void)
{
	...
	for (int nCntAtk_bullet_00_type = 0; nCntAtk_bullet_00_type < ATK_BULLET_00_TYPE_MAX; nCntAtk_bullet_00_type++)
	{
		...
		{
			D3DXCreateTextureFromFile(pDevice, g_aAtk_bullet_00Type[nCntAtk_bullet_00_type].aTexturPath, &g_aTextureAtk_bullet_00[nCntAtk_bullet_00_type]);
		}
	}
	...
	Atk_bullet_00	*pAtk				//
					= g_aAtk_bullet_00;	// 弾[00] の情報のポインタ
...
void UninitAtk_bullet_00(void)
{
	...
	{
		if (g_aTextureAtk_bullet_00[nCntAtk] != NULL)
		{
			g_aTextureAtk_bullet_00[nCntAtk]->Release();
			g_aTextureAtk_bullet_00[nCntAtk] = NULL;
		}
	}
